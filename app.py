...
def get_gallery_folders():
    try:
        ...
        cursor = db.cursor()
        cursor.execute('''
            SELECT f.* FROM gallery_folders f
            JOIN gallery_images i ON f.name = i.folder_name
            GROUP BY f.id
            HAVING COUNT(i.id) > 0
...
def has_images_recursive(folder_id):
    """Recursively check if a folder or its subfolders contain images"""
    if not DRIVE_API_KEY:
        return False
    
    try:
        # Check current folder for images
        url = 'https://www.googleapis.com/drive/v3/files'
        params = {
        ...
        response = requests.get(url, params=params, timeout=10)
        subfolders = response.json().get('files', [])
        
        for sub in subfolders:
            if has_images_recursive(sub['id']):
                return True
                
        return False
    except Exception as e:
        print(f"Recursive check error for {folder_id}: {e}")
        return False
...
def get_drive_folders():
    ...
    try:
        ...
        for folder in folders:
            # Recursive check if folder or subfolders have images
            if not has_images_recursive(folder['id']):
                continue
